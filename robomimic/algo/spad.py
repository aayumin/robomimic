...
import robomimic.models.spad_nets as SPADNets
...
@register_algo_factory_func("spad")
...
class SPADPolicy(PolicyAlgo):
    def _create_networks(self):
        ...
        noise_pred_net = SPADNets.ConditionalUnet1D(
        ...
        # Action encoder
        action_embed_dim = getattr(self.algo_config.spad, "action_embed_dim", obs_cond_dim)
        action_encoder = SPADNets.ActionEncoder(
            action_dim=action_flat_dim,
            feature_dim=action_embed_dim,
            hidden_dims=getattr(self.algo_config.spad, "action_hidden_dims", [512, 512]),
            activation=getattr(self.algo_config.spad, "action_activation", "relu"),
            dropout=getattr(self.algo_config.spad, "action_dropout", 0.0),
            layer_norm=getattr(self.algo_config.spad, "action_layer_norm", True),
            normalize_output=False,
        )


        # Projection heads are used only for InfoNCE / soft contrastive losses.
        obs_projection = SPADNets.ProjectionHead(
            input_dim=obs_cond_dim,
            output_dim=getattr(self.algo_config.spad, "contrast_dim", 128),
            hidden_dim=getattr(self.algo_config.spad, "proj_hidden_dim", 256),
            activation=getattr(self.algo_config.spad, "proj_activation", "relu"),
            layer_norm=getattr(self.algo_config.spad, "proj_layer_norm", True),
            normalize_output=getattr(self.algo_config.spad, "normalize_projection", True),
        )

        action_projection = SPADNets.ProjectionHead(
    ...
    def train_on_batch(self, batch, epoch, validate=False):
        ...
        with TorchUtils.maybe_no_grad(no_grad=validate):
            info = super(SPADPolicy, self).train_on_batch(batch, epoch, validate=validate)
            ...
            z_o = self.nets["policy"]["obs_projection"](obs_cond)
            z_a = self.nets["policy"]["action_projection"](action_features)

            # soft phase-progress contrastive loss
            target_oa, valid_oa = self._spad_build_soft_targets(
                phase_ids=batch["phase_ids"],
                phase_progress=batch["phase_progress"],
                sigma_progress=self.algo_config.spad.sigma_progress,
                exclude_self=False,
            )
            target_intra, valid_intra = self._spad_build_soft_targets(
                phase_ids=batch["phase_ids"],
                phase_progress=batch["phase_progress"],
                sigma_progress=self.algo_config.spad.sigma_progress,
                exclude_self=self.algo_config.spad.exclude_self_for_intra_modal,
            )

            loss_oa = self._spad_soft_contrastive_loss(z_o, z_a, target_oa, valid_oa, self.algo_config.spad.temperature, mask_self=False)
            loss_ao = self._spad_soft_contrastive_loss(z_a, z_o, target_oa.t(), valid_oa, self.algo_config.spad.temperature, mask_self=False)
            loss_oo = self._spad_soft_contrastive_loss(z_o, z_o, target_intra, valid_intra, self.algo_config.spad.temperature, mask_self=True)
            loss_aa = self._spad_soft_contrastive_loss(z_a, z_a, target_intra, valid_intra, self.algo_config.spad.temperature, mask_self=True)

            contrastive_loss = (
                self.algo_config.loss_weight.oa * loss_oa
                + self.algo_config.loss_weight.ao * loss_ao
                + self.algo_config.loss_weight.oo * loss_oo
                + self.algo_config.loss_weight.aa * loss_aa
            )
            ...
            contrastive_weight = self.algo_config.loss_weight.contrastive
            if self.algo_config.loss_weight.aux_decay.enabled:
                if self.algo_config.loss_weight.aux_decay.func == "linear":
                    contrastive_weight = contrastive_weight * float(max(0, self.algo_config.loss_weight.aux_decay.epochs - epoch) / self.algo_config.loss_weight.aux_decay.epochs)
                else:
                    raise RuntimeError(f"Unsupported aux_decay func: {self.algo_config.loss_weight.aux_decay.func}")

            loss = self.algo_config.loss_weight.diffusion * diffusion_loss + contrastive_weight * contrastive_loss
    ...
    def log_info(self, info):
        ...
        """
        log = super(SPADPolicy, self).log_info(info)

        log["Diffusion"] = info["losses"]["diffusion_loss"].item()
        log["Contrastive"] = info["losses"]["contrastive_loss"].item()
        log["OA"] = info["losses"]["loss_oa"].item()
        log["AO"] = info["losses"]["loss_ao"].item()
        log["OO"] = info["losses"]["loss_oo"].item()
        log["AA"] = info["losses"]["loss_aa"].item()
    ...
    def _spad_build_soft_targets(self, phase_ids, phase_progress, sigma_progress=0.25, exclude_self=False, eps=1e-8):
        """
        phase_ids: [B]
    ...
    def _spad_soft_contrastive_loss(self, query, key, target_probs, valid_mask=None, temperature=0.1, mask_self=False):
        """
        query: [B, D]
        key: [B, D]
        target_probs: [B, B]
