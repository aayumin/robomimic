...
import robomimic.models.drcan_nets as DRCANNets
...
@register_algo_factory_func("drcan")
...
class DRCANPolicy(PolicyAlgo):
    def _create_networks(self):
        ...
        noise_pred_net = DRCANNets.ConditionalUnet1D(
            input_dim=self.ac_dim,
            global_cond_dim=obs_dim*self.algo_config.horizon.observation_horizon
        )

        proj_dim = self.algo_config.drcan.proj_dim

        obs_proj = nn.Sequential(
            nn.Linear(obs_dim * self.algo_config.horizon.observation_horizon, proj_dim),
            nn.ReLU(inplace=True),
            nn.Linear(proj_dim, proj_dim),
        )

        denoise_proj = nn.Sequential(
            nn.Linear(self.algo_config.drcan.hidden_dim, proj_dim),
            nn.ReLU(inplace=True),
            nn.Linear(proj_dim, proj_dim),
        )
    ...
    def train_on_batch(self, batch, epoch, validate=False):
        ...
        with TorchUtils.maybe_no_grad(no_grad=validate):
            info = super(DRCANPolicy, self).train_on_batch(batch, epoch, validate=validate)
            ...
            noise_pred, denoise_hidden = self.nets["policy"]["noise_pred_net"](
                noisy_actions, timesteps, global_cond=obs_cond, return_hidden=True,)
            
            # denoise_hidden should become (B, hidden_dim)
            # If your hidden feature is [B, C, T], use mean over T.
            if denoise_hidden.ndim == 3:
                denoise_hidden = denoise_hidden.mean(dim=-1)
            elif denoise_hidden.ndim != 2:
                raise ValueError(f"Unexpected denoise_hidden shape: {denoise_hidden.shape}")

            # projection heads
            obs_proj = self.nets["policy"]["obs_proj"](obs_cond)
            denoise_proj = self.nets["policy"]["denoise_proj"](denoise_hidden)

            # DRCAN contrastive loss
            contrastive_loss = drcan_infonce_loss(
                obs_proj=obs_proj,
                denoise_proj=denoise_proj,
                timesteps=timesteps,
                num_train_timesteps=self.noise_scheduler.config.num_train_timesteps,
                temperature=self.algo_config.drcan.temperature,
                timestep_weight_mode=self.algo_config.drcan.timestep_weight_mode,
            )
            ...
            if "aux_decay" in self.algo_config.loss_weight:
                if self.algo_config.loss_weight.aux_decay.func == "linear":
                    ...
                    contrastive_loss_weight = contrastive_loss_weight * float(
                        max(0, self.algo_config.loss_weight.aux_decay.epochs - epoch)
                        / self.algo_config.loss_weight.aux_decay.epochs
                    )
                else:
                    raise RuntimeError()
            else:
...
def drcan_timestep_weight(
    ...
    tau = timesteps.float() / float(num_train_timesteps - 1)  # [0, 1]

    if mode == "none":
        w = torch.ones_like(tau)
    elif mode == "high":
        # high-noise timestep emphasized
        w = tau
    elif mode == "low":
        # low-noise timestep emphasized
        w = 1.0 - tau
    elif mode == "mid":
        # middle timesteps emphasized
        w = 4.0 * tau * (1.0 - tau)
    else:
        raise ValueError(f"Unknown timestep weight mode: {mode}")

    # normalize so mean weight ~= 1
    return w / w.mean().clamp(min=1e-6)


def drcan_infonce_loss(
    obs_proj: torch.Tensor,
    denoise_proj: torch.Tensor,
    timesteps: torch.Tensor,
    num_train_timesteps: int,
    temperature: float = 0.1,
    timestep_weight_mode: str = "mid",
):
    """
    obs_proj:      (B, D)
    denoise_proj:  (B, D)
    timesteps:     (B,)
    return:        (B,) per-sample loss
    """
    obs_proj = F.normalize(obs_proj, dim=-1)
    denoise_proj = F.normalize(denoise_proj, dim=-1)

    logits = obs_proj @ denoise_proj.transpose(0, 1)   # (B, B)
    logits = logits / temperature

    labels = torch.arange(logits.shape[0], device=logits.device)

    # symmetric InfoNCE
    loss_o2h = F.cross_entropy(logits, labels, reduction="none")      # (B,)
    loss_h2o = F.cross_entropy(logits.transpose(0, 1), labels, reduction="none")  # (B,)
    con_loss = 0.5 * (loss_o2h + loss_h2o)

    w_t = drcan_timestep_weight(
        timesteps=timesteps,
        num_train_timesteps=num_train_timesteps,
        mode=timestep_weight_mode,
    )
