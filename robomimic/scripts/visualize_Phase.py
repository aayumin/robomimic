...
def get_datetime_path_regex(original_path):
    ...
    # 숫자 폴더(/숫자/) 패턴을 찾아서 그 폴더 뒤의 모든 문자열을 제거
    # \d+ : 1개 이상의 숫자, (?=/) : 뒤에 슬래시가 오는 경우
    match = re.search(r'.*/\d+(?=/)', path)
    
    if match:
        return match.group(0) + '/'
    return "타임스탬프 폴더를 찾을 수 없습니다."
...
def run_trained_agent(args):
    ...
    result_dir = os.path.join(get_datetime_path_regex(ckpt_path), "logs", "viz_phase")
    video_path = os.path.join(result_dir, "video_with_phase.mp4")
    ...
    policy, ckpt_dict = FileUtils.policy_from_checkpoint(ckpt_path=ckpt_path, device=device)
